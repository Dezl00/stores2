...
with open("src/app/admin/admin-layout-client.tsx", "r", encoding="utf-8") as f:
    content = f.read()

# 1. Imports
content = re.sub(
    r'import \{ (.*?) \} from "lucide-react"',
    lambda m: f'import {{ {", ".join(list(set(m.group(1).split(", ") + ["ChevronDown", "Home", "Megaphone", "Store", "Shield"])))} }} from "lucide-react"',
    content
)
...
# 3. Add NavGroup component
navGroupCode = """function NavGroup({ group, pathname, isMobile, setMobileMenuOpen }: { group: any, pathname: string, isMobile?: boolean, setMobileMenuOpen?: (open: boolean) => void }) {
...
# 4. Replace the old nav block using a regex since we don't know exact spacing
old_nav_pattern = r'<nav className="p-4 space-y-6 flex-1 overflow-y-auto">.*?</nav>'

new_desktop_nav = """<nav className="p-4 flex-1 overflow-y-auto custom-scrollbar">
          {navGroups.map((group) => (
            <NavGroup key={group.title} group={group} pathname={pathname} isMobile={false} />
          ))}
        </nav>"""

new_mobile_nav = """<nav className="p-4 flex-1 overflow-y-auto custom-scrollbar">
          {navGroups.map((group) => (
            <NavGroup key={group.title} group={group} pathname={pathname} isMobile={true} setMobileMenuOpen={setIsMobileMenuOpen} />
          ))}
        </nav>"""

matches = re.findall(old_nav_pattern, content, flags=re.DOTALL)
if len(matches) == 2:
    content = content.replace(matches[0], new_desktop_nav, 1)
    content = content.replace(matches[1], new_mobile_nav, 1)
else:
    print("WARNING: Did not find exactly 2 matches!")

with open("src/app/admin/admin-layout-client.tsx", "w", encoding="utf-8") as f:
    f.write(content)
