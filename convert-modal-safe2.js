...
// We need to match the entire `if (editingItem) { ... }` block
// We can use a trick: split by `if (editingItem) {`
const parts = content.split('if (editingItem) {\n    return (');
if (parts.length < 2) {
...
// Find where this block ends. It ends before `return (\n      <div className="flex flex-col h-full bg-white relative`
...
let jsxContent = '<div className="flex flex-col h-full bg-white relative' + mainReturnParts[0];
// Remove the trailing `    }\n  ` from jsxContent
...
const finalIndex = mainReturn.lastIndexOf(finalClosing);
if (finalIndex !== -1) {
  mainReturn = mainReturn.substring(0, finalIndex) + modalHTML + "\n      " + finalClosing + mainReturn.substring(finalIndex + finalClosing.length);
}
