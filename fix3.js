...
// createWidget exactly
content = content.replace(/(export async function createWidget\([^)]*\)\s*\{\s*try\s*\{\s*const storeId = await resolveStoreId\(\))/g, 
...
content = content.replace(/(export async function deleteWidget\([^)]*\)\s*\{\s*try\s*\{\s*const storeId = await resolveStoreId\(\))/g, 
...
content = content.replace(/(export async function createWidgetContentItem\([^)]*\)\s*\{\s*try\s*\{\s*const storeId = await resolveStoreId\(\))/g, 
    '$1\n    await requireStoreAdmin()');

content = content.replace(/(export async function deleteWidgetContentItem\([^)]*\)\s*\{\s*try\s*\{\s*const storeId = await resolveStoreId\(\))/g, 
    '$1\n    await requireStoreAdmin()');
...
content = content.replace(/(export async function updateWidgetContentItem\([^)]*\)\s*\{\s*try\s*\{\s*const storeId = await resolveStoreId\(\))/g, 
    '$1\n    await requireStoreAdmin()');
...
content = content.replace(/(export async function updateWidgetContentItemOrder\([^)]*\)\s*\{\s*try\s*\{\s*const storeId = await resolveStoreId\(\))/g, 
    '$1\n    await requireStoreAdmin()\n    for (const update of updates) {\n      const existing = await db.widgetContentItem.findFirst({ where: { id: update.id, widget: { storeId } } })\n      if (!existing) throw new Error("Not found")\n    }');

content = content.replace(/(export async function saveThemeSettings\([^)]*\)\s*\{\s*try\s*\{\s*const storeId = await resolveStoreId\(\))/g, 
    '$1\n    await requireStoreAdmin()');
