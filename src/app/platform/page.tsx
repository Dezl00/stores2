...
export default async function PlatformDashboard() {
  ...
  const recentStores = await db.store.findMany({
    ...
    take: 5,
    select: { id: true, name: true, customDomain: true, slug: true, status: true, createdAt: true }
  })
              ...
              {recentStores.map(store => {
                ...
                return (
                  <tr key={store.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-gray-600 font-mono text-sm">{store.customDomain || '-'}</td>
