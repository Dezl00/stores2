  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto" dir="rtl">
        <DialogHeader>
          <DialogTitle>إدارة المتغيرات والخيارات للمنتج</DialogTitle>
          <DialogDescription>
            ...
            <div className="flex flex-wrap gap-2">
              {availableGlobalOptions.map(opt => (
                <Button 
                  ...
                  onClick={() => handleAddGlobalOption(opt)}
                  disabled={loading}
                >
              ...
              return (
                <div key={opt.id} className="border border-border/50 rounded-xl bg-card shadow-sm overflow-hidden">
                  <div className="bg-muted/30 p-3 flex items-center justify-between border-b border-border/50">
                    ...
                    <Button variant="ghost" size="sm" className="text-destructive hover:bg-destructive/10 h-8 px-2" onClick={() => handleDeleteOption(opt.id)}>
                        ...
                        globalOpt.values.map((gVal: any) => {
                          ...
                          return (
                            <button
                              key={gVal.id}
                              disabled={loading}
                              onClick={() => handleToggleValue(opt.id, gVal, isSelected, productVal?.id)}
